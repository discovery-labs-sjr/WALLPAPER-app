...
from app import BASE, Base, SessionLocal, User, WALLPAPER_BY_ID, WALLPAPERS, app, current_user, engine, require_csrf

ADMIN_EMAIL=os.getenv('ADMIN_EMAIL','').strip().lower()
UPLOAD_DIR=BASE/'static'/'uploads'; UPLOAD_DIR.mkdir(parents=True,exist_ok=True)
MAX_BYTES=int(os.getenv('WALLPAPER_MAX_UPLOAD_MB','20'))*1024*1024
...
class WallpaperAsset(Base):
    ...
    id:Mapped[str]=mapped_column(String(80),primary_key=True); slug:Mapped[str]=mapped_column(String(120),unique=True,index=True)
    title:Mapped[str]=mapped_column(String(180)); category:Mapped[str]=mapped_column(String(80)); tags_json:Mapped[str]=mapped_column(Text,default='[]')
    wallpaper_type:Mapped[str]=mapped_column(String(20),default='static'); mime_type:Mapped[str]=mapped_column(String(100))
    width:Mapped[int]=mapped_column(Integer); height:Mapped[int]=mapped_column(Integer); size_bytes:Mapped[int]=mapped_column(Integer)
    image_bytes:Mapped[bytes]=mapped_column(LargeBinary); created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=lambda:datetime.now(timezone.utc))
...
def fallback(title:str,name:str,tags:list[str])->tuple[str,float]:
    hay=' '.join([title,name,*tags]).lower(); scores={c:sum(k in hay for k in ks) for c,ks in KEYWORDS.items()}; cat,score=max(scores.items(),key=lambda x:x[1])
    return (cat,round(min(.94,.45+score*.14),2)) if score else ('Aesthetic',.38)
...
def item(row:WallpaperAsset)->dict[str,Any]:
    ...
    return {'id':row.id,'slug':row.slug,'title':row.title,'category':row.category,'tags':json.loads(row.tags_json or '[]'),'type':row.wallpaper_type,'preview':public,'file':public,'source':'creator','width':row.width,'height':row.height,'size_bytes':row.size_bytes}
...
def load_assets():
    ...
    for row in rows:
        ...
        else: WALLPAPERS.append(x); WALLPAPER_BY_ID[row.id]=x

@app.get('/admin',response_class=HTMLResponse)
def admin_page()->str:return (BASE/'static/admin.html').read_text(encoding='utf-8')
@app.get('/api/admin/me')
def admin_me(user:User=Depends(admin_user)):return {'ok':True,'email':user.email,'categories':CATEGORIES}
@app.get('/api/admin/wallpapers')
def admin_list(user:User=Depends(admin_user)):
    with SessionLocal() as db: rows=db.scalars(select(WallpaperAsset).order_by(WallpaperAsset.created_at.desc())).all()
    return [item(r) for r in rows]
@app.post('/api/admin/analyze')
async def admin_analyze(image:UploadFile=File(...),title:str=Form(''),tags:str=Form(''),_:None=Depends(require_csrf),user:User=Depends(admin_user)):
    data=await image.read(); q=check_image(data,image.filename or 'wallpaper.jpg'); taglist=[x.strip() for x in tags.split(',') if x.strip()][:12]; cat,conf=fallback(title,image.filename or '',taglist); ai,ais=await ai_category(data)
...
async def admin_upload(image:UploadFile=File(...),title:str=Form(...),category:str=Form('Aesthetic'),tags:str=Form(''),wallpaper_type:str=Form('static'),_:None=Depends(require_csrf),user:User=Depends(admin_user)):
    ...
    data=await image.read(); q=check_image(data,image.filename or 'wallpaper.jpg'); title=title.strip()
    ...
    row=WallpaperAsset(id=wid,slug=f'{slug}-{wid[-6:]}',title=title,category=category,tags_json=json.dumps([x.strip() for x in tags.split(',') if x.strip()][:12],ensure_ascii=False),wallpaper_type=wallpaper_type,mime_type=q['mime_type'],width=q['width'],height=q['height'],size_bytes=q['size_bytes'],image_bytes=data)
    with SessionLocal() as db:db.add(row);db.commit();db.refresh(row)
    x=item(row);WALLPAPERS.append(x);WALLPAPER_BY_ID[row.id]=x
