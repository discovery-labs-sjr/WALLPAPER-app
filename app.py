...
with DATA.open("r", encoding="utf-8") as handle:
    WALLPAPERS = json.load(handle)

WALLPAPER_BY_ID = {item["id"]: item for item in WALLPAPERS}
WALLPAPER_IDS = set(WALLPAPER_BY_ID)
ALLOWED_IMAGE_HOSTS = {"images.unsplash.com", "images.pexels.com"}

app = FastAPI(title="WALLVERSE", version="7.0.0")
...
def clear_auth_cookies(response: Response) -> None:
    for name in ("wallverse_access", "wallverse_refresh", "wallverse_csrf"):
...
def current_user(db: Session, access_cookie: str | None) -> User | None:
...
def require_user(
    wallverse_access: str | None = Cookie(default=None, alias="wallverse_access"),
) -> User:
...
def require_csrf(
    request: Request,
    csrf_cookie: str | None = Cookie(default=None, alias="wallverse_csrf"),
) -> None:
...
def local_wallpaper_file(item: dict) -> tuple[Path, str]:
    raw_file = str(item.get("file") or "")
    relative = Path(raw_file.lstrip("/"))
    if relative.is_absolute() or ".." in relative.parts:
        raise HTTPException(400, "Fichier wallpaper invalide")
...
def remote_preview_url(item: dict) -> str | None:
...
def get_wallpaper(wallpaper_id: str) -> dict:
...
async def download_wallpaper(wallpaper_id: str):
    ...
    if preview and _http_client:
        try:
            ...
            if remote.is_success:
                media_type = remote.headers.get("content-type") or mimetypes.guess_type(preview)[0] or "image/jpeg"
                return Response(
                    content=remote.content,
                    media_type=media_type,
                    headers={
                        "Content-Disposition": f'attachment; filename="{wallpaper["slug"]}.jpg"',
    ...
    file_path, media_type = local_wallpaper_file(wallpaper)
    filename = f'{wallpaper["slug"]}{file_path.suffix or ".svg"}'
...
def csrf_token(
    response: Response,
    wallverse_csrf: str | None = Cookie(default=None, alias="wallverse_csrf"),
) -> dict:
...
def register(data: RegisterIn, response: Response, _: None = Depends(require_csrf)) -> dict:
...
def login(data: LoginIn, response: Response, _: None = Depends(require_csrf)) -> dict:
    email = data.email.lower().strip()
...
def refresh(
    response: Response,
    wallverse_refresh: str | None = Cookie(default=None, alias="wallverse_refresh"),
    _: None = Depends(require_csrf),
) -> dict:
    if not wallverse_refresh:
        raise HTTPException(401, "Session expirée")

    with SessionLocal() as db:
        session = db.scalar(
            select(AuthSession).where(
                AuthSession.refresh_hash == token_hash(wallverse_refresh),
        ...
        issue_session(db, session.user, response)
        return {"ok": True}
...
@app.get("/api/auth/me")
def me(wallverse_access: str | None = Cookie(default=None, alias="wallverse_access")) -> dict:
    with SessionLocal() as db:
        user = current_user(db, wallverse_access)
        if not user:
            return {"authenticated": False}

        favorite_ids = db.scalars(select(Favorite.wallpaper_id).where(Favorite.user_id == user.id)).all()
        return {
            "authenticated": True,
            "user": {
                "id": user.id,
...
def get_favorites(user: User = Depends(require_user)) -> list[str]:
    with SessionLocal() as db:
        return list(db.scalars(select(Favorite.wallpaper_id).where(Favorite.user_id == user.id)).all())
...
def add_favorite(
    ...
    if data.wallpaper_id not in WALLPAPER_IDS:
        raise HTTPException(404, "Wallpaper introuvable")
    ...
    with SessionLocal() as db:
        existing = db.scalar(
            select(Favorite).where(
                ...
                Favorite.wallpaper_id == data.wallpaper_id,
            )
        )
        if not existing:
            db.add(Favorite(user_id=user.id, wallpaper_id=data.wallpaper_id))
            db.commit()
