...
def enhance_image(data:bytes)->tuple[bytes,dict[str,Any]]:
    im=open_image(data); original_size=im.size
    ...
    im=im.filter(ImageFilter.UnsharpMask(radius=1.15,percent=105,threshold=3))
    out=io.BytesIO(); im.save(out,format='WEBP',quality=94,method=6)
    result=out.getvalue()
...
def studio_me(user:User=Depends(admin_user)):return {'ok':True,'email':user.email,'categories':CATEGORIES,'enhancement':{'enabled':True,'max_long_side':ENHANCE_MAX_LONG}}
...
async def studio_enhance(image:UploadFile=File(...),_:None=Depends(require_csrf),user:User=Depends(admin_user)):
    ...
    return {'ok':True,'enhancement':meta,'mime_type':'image/webp','preview_data':'data:image/webp;base64,'+__import__('base64').b64encode(result).decode('ascii')}
...
async def studio_upload(image:UploadFile=File(...),title:str=Form(...),category:str=Form('Aesthetic'),tags:str=Form(''),wallpaper_type:str=Form('static'),enhance:str=Form('true'),_:None=Depends(require_csrf),user:User=Depends(admin_user)):
    ...
    wid=f'wv-{datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S")}-{secrets.token_hex(3)}';slug=re.sub(r'[^a-z0-9]+','-',title.lower()).strip('-')[:80] or 'wallpaper';row=WallpaperAsset(id=wid,slug=f'{slug}-{wid[-6:]}',title=title,category=category,tags_json=json.dumps([x.strip() for x in tags.split(',') if x.strip()][:12],ensure_ascii=False),wallpaper_type=wallpaper_type,mime_type=q['mime_type'],width=q['width'],height=q['height'],size_bytes=q['size_bytes'],image_bytes=data)
