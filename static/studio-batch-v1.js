(() => {
  ...
  function mount() {
    ...
    panel.innerHTML = `
      <div class="batchHead"><div><p class="batchEyebrow">PUBLICATION RAPIDE</p><h2>Publier plusieurs contenus</h2><p>Ajoute jusqu’à 20 images. Les profils sont automatiquement cadrés et enregistrés dans une rail séparée.</p></div><span class="batchBadge">LOT · RAPIDE</span></div>
      <div class="batchControls">
        ...
        <label id="batchCropLabel">Cadrage<select id="batchCrop"><option value="square">Carré — avatar</option><option value="portrait">Portrait 4:5</option><option value="original">Original</option></select></label>
      </div>
  ...
  function addFiles(fileList) {
    ...
    state.files = [...state.files, ...incoming].filter((file, index, all) => index === all.findIndex(other => other.name === file.name && other.size === file.size)).slice(0, 20);
    renderQueue();
    $('#batchFiles').value = '';
  }
  ...
  function renderQueue() {
    ...
    queue.innerHTML = state.files.map((file, index) => `<article class="batchFile"><img src="${URL.createObjectURL(file)}" alt=""><div><strong>Image ${index + 1}</strong><small>${(file.size / 1048576).toFixed(1)} Mo</small></div><button class="batchRemove" type="button" data-index="${index}" aria-label="Retirer">×</button></article>`).join('');
  ...
  async function publishBatch() {
    ...
    form.append('crop_mode', $('#batchCrop').value);
